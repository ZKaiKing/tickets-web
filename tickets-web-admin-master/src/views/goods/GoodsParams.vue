  ...
  <div>
      ...
      <Row>
        <Form ref="searchForm" :model="searchForm" inline :label-width="70" class="search-form">
          <Form-item label="参数名称" prop="name">
            <Input type="text" v-model="searchForm.name" clearable placeholder="请输入商品参数名称" style="width: 200px" />
          </Form-item>
    ...
    <Modal :title="modalTitle" v-model="modalVisible" :mask-closable="false" :width="500" :style="{top:'30px'}">
      <Form ref="dataOperForm" :model="dataOperForm" :label-width="100" :rules="dataOperFormValidate">
        <FormItem label="商品参数名：" prop="name">
  ...
  export default {
    data() {
      return {
        ...
        columns: [
          ...
          { title: '商品参数名称', key: 'name', align: 'center', minWidth: 200, sortable: true },
        ...
        dataOperFormValidate: {
          name: [
            { required: true, message: "商品参数名称不能为空", trigger: "change" }
          ],
          enabled: [
    ...
    methods: {
      ...
      deteleData(row) {
        this.$Modal.confirm({
          title: "提示",
          content: '请确认是否要删除商品参数:  <strong>' + row.name + '</strong> ?',
          onOk: () => {
      ...
      enabledData(row) {
        ...
        this.$Modal.confirm({
          title: "提示",
          content: '请确认是否要' + option + '商品参数:  <strong>' + row.name + '</strong> ?',
          onOk: () => {
            let ids = [];
            ids.push(row.id);
